...
* IF combination loses the fight for the player
    IF cost for combination is higher than the current highest
        THEN store this cost
    ELSE move to the next combination
ELSE move to the next combination
RETURN the highest cost

* at the start of the line shows change to the part 1 solution
*/
...
namespace aoc21b
{
    ...
    bool doesPlayerLose(const Character& boss, const Character& player)
    {
    ...
    GoldInt findDearestLoss(const Character& boss, const Weapons& weapons, const Armours& armours, const Rings& rings)
    {
        GoldInt dearest {};
            ...
            for (int a{-1}; a < static_cast<int>(armours.size()); ++a)
            {
                for (int r1{-1}; r1 < static_cast<int>(rings.size()); ++r1)
                {
                    for (int r2{r1 + 1}; r2 < static_cast<int>(rings.size()); ++r2)
                    {
                        ...
                        if (doesPlayerLose(boss, player) && totalCost > dearest)
                        {
                            dearest = totalCost;
                        }
                    }
    ...
    GoldInt getMostGoldToLose(const Input<L>& boss, const Input<M>& shop)
    {
        ...
        return findDearestLoss(bossStats, weapons, armours, rings);
    }
}
