...
/*
there may be a lot of combinations to try
would it be easier to create a class to track effects?

given logic below, a recursive function may be needed
...
namespace aoc22a
{
    ...
    Mana recursiveMinimumMana(Player player, Boss boss, const SpellCosts& spellCosts, size_t spellIndex, SpellTurns spellTurns, Mana manaSpent, Mana& minMana)
    {
        ...
        {
            if ((spellIndex > 1 && spellTurns.at(spellIndex - 2) > 1) || (player.second < spellCosts.at(spellIndex) + ((spellTurns.at(2) == 0) ? 0 : RECHARGE_REGAIN_PER_TURN)))
                continue;

            // only continue if there's any point
            if (player.first > 0 && boss.first > 0 && (minMana == 0 || manaSpent < minMana))
                recursiveMinimumMana(player, boss, spellCosts, spellIndex + 1, spellTurns, manaSpent, minMana);
            
            // player turn
            applySpellEffectsAndReduceTurn(player, boss, spellTurns);
            
            if (boss.first <= 0 && player.first > 0 && (minMana == 0 || manaSpent < minMana) && player.second >= 0)
            {
                minMana = manaSpent;
            }
            
            manaSpent += spellCosts.at(spellIndex);
            player.second -= spellCosts.at(spellIndex);
            switch (spellIndex)
            {
            case 0:
